...
contract LTO
{
    ...
    function getppdata(int _index)public view returns(string memory due,int payment,int penalty,string memory statusl,int vno)
    {
        return (myMapping1[_index].due,myMapping1[_index].payment,myMapping1[_index].penalty,myMapping1[_index].statusl,myMapping[_index].vno);
    }
    
    function payment(int _vno,int _penalty)public
    {
        for(int x=0;x < index;x++)
        {
            if(myMapping[x].vno == _vno)
            {
                myMapping1[x].penalty = _penalty;
                myMapping[x].status = true;
            }
        }
    }
     function tracking(int _dno,int _vno,string memory code)public
    {
        if(keccak256(bytes(code)) == keccak256(bytes("Onhand")))
        {
        for(int x=0;x < index;x++)
        {
                if(myMapping[x].dno == _dno)
                {
                    myMapping1[x].statusl = code;
                }
        }
        }
        else
